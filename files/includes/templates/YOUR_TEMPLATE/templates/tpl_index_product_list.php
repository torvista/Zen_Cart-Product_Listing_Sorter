...
<?php
  ...
  if ($do_filter_list || ($check_for_alpha->RecordCount() > 0 && (PRODUCT_LIST_ALPHA_SORTER == 'true' || PRODUCT_LISTING_SORTER == 'true'))) {
  $form = zen_draw_form('filter', zen_href_link(FILENAME_DEFAULT), 'get') . ($do_filter_list || ($check_for_alpha->RecordCount() > 0 && PRODUCT_LIST_ALPHA_SORTER == 'true') ? '<label class="inputLabel">' .TEXT_SHOW . '</label>':'');
?>
