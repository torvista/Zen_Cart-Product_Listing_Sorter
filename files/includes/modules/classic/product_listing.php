<?php
//MOD Product Listing Sorter: this change is to preserve the sort order after clicking on a product in a list, looking at the product, then returning to the list.
  ...
  while (!$listing->EOF) {
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        case 'PRODUCT_LIST_NAME':
        $lc_align = '';
		//MOD Product Listing Sorter 2 of 2
        $lc_text = '<h3 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id'] > 0) ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id'].'&'.zen_get_all_get_params(array('cPath'))) . '">' . $listing->fields['products_name'] . '</a></h3><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>';//MOD Product Listing Sorter, added GETs to carry selected filter parameters
        //eof PLS
		break;
        case 'PRODUCT_LIST_MANUFACTURER':
        $lc_align = '';
