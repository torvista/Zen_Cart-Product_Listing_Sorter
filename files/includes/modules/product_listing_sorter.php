...
//steve hack for adding category-specific options
    if (strncmp($cPath, '3_', 2) === 0) {//everything under bikes
        $product_sorter_list = explode(';', $prefix . trim(PRODUCT_LISTING_SORTER_LIST_BIKES));
    } else {
    $product_sorter_list = explode(';', $prefix . trim(PRODUCT_LISTING_SORTER_LIST));
    }
    $product_sorter_options_list = [];
    if ($product_sorter_list !== false && is_array($product_sorter_list)) {
    ...
    } elseif ($debug_pls) $debug_pls_msg .= 'product_listing_sorter ' . __LINE__ . ': array $product_sorter_list not created <br>';
